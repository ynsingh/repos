        <g:if test="${contentDetails}">
               <g:if test="${contentDetails.contentMode == 'T'}">
                   <div ailgn="center" class="divContent">
                        ${contentText.decodeHTML()}
                   </div>
                </g:if>
                <g:elseif test="${contentDetails.contentMode== 'L'}">
                  <iframe inLoad="iFrameHeight();" src="${contentDetails.contentDescription}" scrolling="no" 
                    scroll="auto" id="iframename" marginwidth="0" marginheight="0" 
                    frameborder="0" style="width:800px;" height=600></iframe>
                </g:elseif>
                <g:elseif test="${contentDetails.contentMode== 'Q'}">
                <div class="divQuiz">
                  <div id="displayQuiz"><br>
           ...
           <form name="mchForm" id="mchForm">
           <g:each in="${questionContentList}" var="i" status="ik">
           <g:if test="${i.type!=3}" >
               <input type="hidden" id="headerStatus" value="1"/>
              <g:set var="QuestionCount" value="${ik+1}" />
           </g:if>
            <g:if test="${i!=[]}" >
            
           <input type="hidden" value="N" name="hintUsed${i.qnId}" id="hintUsed${i.qnId}"/>
           <input type="hidden" value="${i.qnId}" class="mChQnid" id="mChQnid${ik}"/>
           <!-- new design  -->
           <g:if test="${flash.checked}"> 
            <g:set var="pos" value="${flash.checked.indexOf('Q'+i.qnId)}"/> 
            <g:set var="posEqual" value="${flash.checked.indexOf('=', pos) }"/> 
            <g:set var="strLength" value="${ flash.checked.substring(pos+1,posEqual).length()}"/> 
            <g:set var="pos_" value="${flash.checked.indexOf('_', pos) }"/> 
            <g:set var="checkedValue" value=" ${ flash.checked.substring(pos+strLength+2,pos_)}"/> 
           </g:if>
               <g:if test="${i.type == 1||i.type ==3}" >
                     <table id="mChTable" border="0" style="width:100%"  onmouseover="document.getElementById('hintDiv${i.qnId}').style.display = 'block';" onmouseout="document.getElementById('hintDiv${i.qnId}').style.display = 'none';">
                         <tr>
                                    <td style="width:2%;vertical-align:top;">
                                     ...
                                     <td width="5%" style="width:1%;vertical-align:top;" >
                                         <span id="questionNum">${count})</span></b>
                                     </td>
                                      </g:if>
                                     <td width="70%" style="vertical-align:top;" colspan="2">
                                     <span id="question">${i.qnText}</span>  
                                     </td>
                                     <g:if test="${i.qnHint}" >
                                     <td valign="top" style="padding-right:60px;" colspan="3" width="12%" height="30px;">
                                         <g:if test="${!flash.submitMessage}">
                                           <div style="display:none" id="hintDiv${i.qnId}"><img  align="right" height="18"  onclick="javascript:showHint('toggleText${i.qnId}','hintUsed${i.qnId}')" src="${hostname}/images/bulb.gif" style="cursor:pointer"/></div>
                                   </g:if>
                                     </td>
                                     </g:if>
                                     <g:else>
                                 ...
                                 <tr>
                                              ...
                                              <td align="left" style="width:3%">
                                                    <img width="100" height="100" src="${hostname}/uploads/quiz/crop/${i.image}" />
                                              </td>
                                              </g:if>
                                              <g:if test="${i.image}" >
                              ...
                              <g:if test="${flash.submitMessage&&checkedValue}">
                                    ...
                                    <g:if test="${flaglength==1||i.answers.size()>4||flaglength!=1&&i.answers.size()==3 }">
                                       <tr>                             
                                  <td align="right" width="32px" id="textAnsChoice" style="vertical-align:middle">
                                      <g:radio id="${j.text}" checked="${check}" name="myAns${i.qnId}"  value="${j.key }"/>
                                  </td>
                                  <td id="textAnsChoice" style="padding-left:3px;">
                                      <label for="ans${j.key }">${j.text }</label>
                                  </td>
                                </tr>
                                    </g:if>
                                 ...
                                 <td  id="textAnsChoice" style="padding-left:20px;" ${align} ${width}>
                                       <g:radio id="${j.text}" checked="${check}" name="myAns${i.qnId}"  value="${j.key }"/>
                                     <label for="ans${j.key }">${j.text }</label>
                                  </td>
                                  <g:if test="${temp%2==0}">
                                           ...
                                           <g:if test="${flash.submitMessage&&checkedValue}">
                                                ...
                                                <g:if test="${j.text }" >
                                                    <td align="center" width="5%">
                                                      <label for="ans${j.key }" >
                                                          <img width="100" height="100" src="${hostname}/uploads/quiz/crop/${j.text }" />
                                                       </label>
                                                       <br />
                                                       <g:radio id="${j.text}" checked="${check}" name="myAns${i.qnId}"  value="${j.key }"/>
                                                                     </td>
               ...
               <g:if test="${flash.submitMessage}">
                   <g:if test="${ flash.answer.substring(x-5,x)!='succe'}">  
                        <g:if test="${i.qnHint}">
                            <div class="divQuizHint"><p id="hint"><i>hint: ${i.qnHint}</i> </p></div>
                        </g:if>
                                </g:if>
                </g:if>
                <div class="divQuizHint" id="toggleText${i.qnId}" style="display:none;"><p id="hint"><i>hint:${i.qnHint}</i></p></div>
           <!-- new design -->
         ...
         <g:if test="${QuestionCount>0}">
          <p>&nbsp;</p>
            <div style="padding-left:40px;padding-bottom:15px;">
                      <input type="button" name="submit" id="mchsubmit" value="Submit"   onclick="quizAnswer();" style="cursor:pointer;width:80px;height:25px;">
                  </div> 
                 </g:if>
           </g:if>
      </div>
