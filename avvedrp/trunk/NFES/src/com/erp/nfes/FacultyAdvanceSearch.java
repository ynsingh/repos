...
public class FacultyAdvanceSearch extends HttpServlet{
...
*/	
	...
	ResultSet rs=stSearchTabs.executeQuery(sql);
		...
		out.println("<HTML><HEAD><TITLE>Search</TITLE>");
  	    ...
  	    out.println("<form method=\"post\" action=\"./SearchFaculty\"  NAME=\"searchForm\" ID=\"search_form\" >");
...
//			cs.close();
			...
			*/
			Statement stSearchTabs1=conn.createStatement();
			String sql1="";
			ResultSet rsTabs1=stSearchTabs.executeQuery("SELECT TABLE_NAME FROM information_schema.tables WHERE table_name LIKE 'staff_profile_%_items'  AND table_name<>'staff_profile_report_v0_items' AND TABLE_SCHEMA=(select database())");	
			while (rsTabs1.next()){
				if (sql1.equals("")==false){
					sql1	=sql + " UNION ";
				}
				sql1=sql1 + " SELECT '" +rsTabs1.getString("TABLE_NAME") +"' tabl ,NAME FROM  " + rsTabs1.getString("TABLE_NAME");						
			}
			//System.out.println(sql);
			ResultSet rslstfld=stSearchTabs1.executeQuery(sql);
