...
public class RecordTypeSettingServlet extends HttpServlet{
	...
	private void initPage(HttpServletRequest request, HttpServletResponse response,MultiLanguageString ml) throws IOException{
		...
		out.println("<td>"+ml.getValue("institution_starts_with")+"</td><td><Input Type=\"text\" SIZE=\"35\" id=\"institution\" name=\"institution\"></td></tr>");

		out.println("<tr><td>"+ml.getValue("department_starts_with")+"</td><td><Input Type=\"text\" SIZE=\"35\" id=\"department\" name=\"department\"></td>");

		out.println("<td>"+ml.getValue("faculty_name_starts_with")+"<td><Input Type=\"text\" SIZE=\"35\" id=\"username\" name=\"username\"></td></tr><tr>");
		...
		out.println("</select>");
		out.println("</td></tr>");
		out.println("<tr><td>"+ml.getValue("filter_by")+"</td>");
		out.println("<td>");
	...
	private void showRecords(HttpServletRequest request, HttpServletResponse response,MultiLanguageString ml) throws IOException, SQLException{
		...
		while(rsentity_det.next()){
			...
			out.println("<TD  class=\"" +classname +"\"><a href=\"#\" onclick=\"showdetals("+userid+","+document_id+",'"+entity_type+"')\">");
