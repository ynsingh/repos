...
$CONSUMER_KEY = 'vladmin.balavidya.com';
$CONSUMER_SECRET = 'oRUdpJk6vhzlxcU3xuxbP70c';
...
$openid_params = array(
  ...
  'openid.return_to'         => "http://{$_SERVER['HTTP_HOST']}{$_SERVER['PHP_SELF']}",
...
if (isset($_REQUEST['popup']) && !isset($_SESSION['redirect_to'])) {
 ...
 $_SESSION['redirect_to'] = "http://{$_SERVER['HTTP_HOST']}{$_SERVER['PHP_SELF']}?{$query_params}";
...
switch(@$_REQUEST['openid_mode']) {
  ...
  case 'checkid_immediate':
    ...
    if ($identifier) {
      ...
      if (!$endpoints) {
		  header("Location:../../login_page.php?msg=No OpenID endpoint found&action=cancelled");
    ...
    break;
  case 'cancel':
  header("Location:../../login_page.php?msg=Sign-in was cancelled&action=cancelled");
...
<?php if(@$_REQUEST['openid_mode'] === 'id_res'): ?>
  ...
  <?php 
  $_SESSION['sessEmail']=$_REQUEST['openid_ext1_value_email'];
  header("Location:../../login_page.php?email_id={$_REQUEST['openid_ext1_value_email']}&fst={$_REQUEST['openid_ext1_value_first']}&lst={$_REQUEST['openid_ext1_value_last']}")?>
<?php endif; ?>
