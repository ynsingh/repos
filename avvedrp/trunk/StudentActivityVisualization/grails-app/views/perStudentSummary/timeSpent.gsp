<html>
    ...
    <body>
        ...
        <div class="body">
            <h1>Students Time Utilization</h1>
            ...
            <table algn="center"><tr><td algn="center">
	...
	<%
		...
		for(int i=0;i<=maxValue+20;i=i+5)
		{
    ...
    <chart:barChart title='Time Spend By the Student' size="${[475,475]}" colors="FF0000,00ff00" type="bvg"
			labels="${labels}" zeroLine="${'0'}" axes="x,y" axesLabels="${axesLabelsTime}"  
			fill="${'bg,s,efefef'}" fill="${'bg,s,efefef'}" dataType='simple' data='${timeSpent}'  />
     ...
     <gchart:radar title='Time Spend By the Student Chart' size="${[475,475]}" colors="${colors}" 
		axes="x,y" axesLabels="${axesLabelsRadarView}" fill="${'bg,s,efefef'}" dataType='text' data='${timeSpent}'/>
