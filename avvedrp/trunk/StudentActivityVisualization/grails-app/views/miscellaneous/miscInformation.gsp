...
<div class="demo"><!-- Tab Area Start -->
         ...
         jQuery(document).ready(function() {
			 jQuery('#tabs').tabs();
                          jQuery('#tabs').bind('tabsselect', function(event, ui) {
                                 ...
                                 // alert(selected);                                  
                                    switch (selected)
                                            {
                                              case 0:
                                                var refresh_url='${createLink(controller:'miscellaneous',action:'firstTab')}';
                                                //alert(refresh_url);
                                                jQuery('#frame1').attr('src',refresh_url);
                                              break; 
                                              case 1:
                                                var refresh_url='${createLink(controller:'miscellaneous',action:'secondTab')}';
                                                //alert(refresh_url);
                                                jQuery('#frame2').attr('src',refresh_url);
                                              break;  
                                              case 2:
                                                var refresh_url='${createLink(controller:'miscellaneous',action:'thirdTab')}';
                                                //alert(refresh_url);
                        ...
                        <li><a href="#tabs-3">INFRASTRUCTURE</a></li>
                        </ul>

                        <div id="tabs-1">
                        <iframe  id="frame1" frameborder="0"  allowTransparency="true" STYLE="background-color: transparent" src="${createLink(controller:'miscellaneous',action:'firstTab')}" width=100% height= 200%>                  
                        </iframe>   
                        </div>
