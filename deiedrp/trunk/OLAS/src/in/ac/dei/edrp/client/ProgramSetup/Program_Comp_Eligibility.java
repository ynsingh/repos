...
public class Program_Comp_Eligibility {
    ...
    public VerticalPanel onModuleLoad() {
        ...
        flex3.setWidget(1, 2, new Label(cons.component()));
        flex3.setWidget(1, 3, componentCombo);
        flex3.setWidget(2, 0, new Label(cons.category()));
        flex3.setWidget(2, 1, category_obj1.categoryCB);
        flex3.setWidget(2, 2, new Label(cons.cutoffPercent()));
        flex3.setWidget(2, 3, cutoffNF);
        flex3.setWidget(3, 2, submitButton);
                                    ...
                                    connectTemp.CheckComponents(entity_id, progID,
                                        new AsyncCallback<Boolean>() {
                                            ...
                                            public void onSuccess(Boolean result) {
                                                if (result == false) {
                                                    MessageBox.alert(msg.noComponentSpecified(
                                                            Program));
                                                } else {
...
//
        submitButton.addListener(new ButtonListenerAdapter() {
                public void onClick(Button button, EventObject e) {
                    ...
                    check = check + markIV(entityTypeCB.entityDescCB) +
                        markIV(entityNameCB.entityCombo) +
                        markIV(componentCombo) + markIV(programCB.progCombo) +
                        markIV(category_obj1.categoryCB) + markIV(cutoffNF);
                        			 ...
                        			 setCallback(new MessageBox.PromptCallback() {
                                        public void execute(String btnID,String text) {
                                            if (btnID.equals("yes")) {
                                            	...
                                            	System.out.println("do nothing since no is clicked");                                                
                                            }
                                        }
                                    });
                            }
...
//
    public void sendAgeDetails(final String[] details) {
        if (checkTf(cutoffNF1) == 0) {
            connectTemp.componentEligibility(details,
