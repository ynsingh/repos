...
public class CM_boardImpl extends RemoteServiceServlet implements CM_boardConnect{
	 ...
	 */
	public CM_entityInfoGetter[] methodGetProgOfferingEntityList(String user_id,String program_id) throws Exception
	{
		try{
			List<?> list=client.queryForList("progOfferingEntity",program_id);
...
//			throw new Exception(e);
	 ...
	 */
	public CM_progMasterInfoGetter[] methodbranchList(String program_id,String entity_id)
	{
		try{
			CM_progMasterInfoGetter CMPMIG=new CM_progMasterInfoGetter();
...
//			throw new Exception(e);
	 ...
	 */
	public CM_boardNormalizationGetter[] methodComponentList(String program_id,String entity_id,String branch_code)
	{
		try{
			CM_progMasterInfoGetter CMPMIG=new CM_progMasterInfoGetter();
			CMPMIG.setProgram_id(program_id);
			CMPMIG.setCreator_id(entity_id);
			CMPMIG.setBranchcode(branch_code);
...
//			throw new Exception(e);
	 ...
	 */
	public void methodAddBoardNormalizationFactor(CM_boardNormalizationGetter object) throws Exception
	{
		try{
			...
			logObj.logger.error("exception in methodAddBoardNormalizationFactor "+e);
			throw e;
		}
	 ...
	 */
	public CM_progMasterInfoGetter[] methodprogListForManage(String user_id)
	{
		try{
			String university_id=user_id.substring(1,5);
			CM_progMasterInfoGetter CMPMIG=new CM_progMasterInfoGetter();
...
//			throw new Exception(e);
	 ...
	 */
	public CM_boardNormalizationGetter[] methodboardListForManage(CM_boardNormalizationGetter object)
	{
		try{			
...
//			throw new Exception(e);
	...
	public void methodNormalizationFactorUpdate(CM_boardNormalizationGetter object)
	{
		try{			
			client.update("updateBoardFactor",object);
				
		}catch (Exception e) {
			logObj.logger.error("exception in methodNormalizationFactorUpdate "+e);
//			throw new Exception(e);
		}
			}
	 ...
	 * Method to update board normalization factor
	 */
	public void methodNormalizationFactorDelete(CM_boardNormalizationGetter object)
	{
		try{			
			client.delete("deleteBoardFactor",object);
			
		}catch (Exception e) {
			logObj.logger.error("exception in methodNormalizationFactorDelete "+e);
//			throw new Exception(e);
		}
