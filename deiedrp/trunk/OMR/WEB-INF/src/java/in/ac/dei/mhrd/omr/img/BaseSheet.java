 ...
 * @Anshul Agarwal
 * Execute only once
 * This class process the base answer sheet computes the position of circles and their corresponding values
 *
 */
public class BaseSheet {
    /**
     * This function process student detail part of the base sheet.
     * @param ip
     * @param infoLeft
     * @param infoRight
     * @param xrtAvg
    ...
    public static void computeInfoCirclePosition(ij.process.ImageProcessor ip,
        ArrayList<MidPoint> infoLeft, ArrayList<MidPoint> infoRight, float xrtAvg,
        float xleftAvg, int xstart, int xend) {
        int count = 0;
        int c = 0;
        int total_circle = 0;
        ...
        for (int j = 0; j < infoLeft.size(); j++) {
            MidPoint ymidLeft = infoLeft.get(j); // retrieve mid point of left blocks
            MidPoint ymidRight = infoRight.get(j); // retrieve mid point of left blocks
            /**
            ...
            for (int i = xstart; i < xend; i++) {
                ...
                if (ip.getPixelValue(i, y) == 0) { // search for black pixel
                	                    if (checkPixelGroup(ip, i, y)) // search for a group of black pixel
                     {
                    	
                        int sp = i; // starting and ending position of the circle

                        int starting_point = sp;
                       

                        //compute the ratio of starting point of the circle
                        posStartratio = ((starting_point - xleftAvg) / (xrtAvg -
                            starting_point));

                        i++;

                        while (ip.getPixelValue(i, y) == 0) // find the end point of the circle
                         {
                       ...
                       // System.out.println("starting point : " + starting_point + " end point : "+ end_point +"circle width : " + (sp-starting_point));
                        		

                        // computes the ratio of end point
                        posEndratio = ((end_point - xleftAvg) / (xrtAvg -
                            end_point)); // computes the ratio of end position of the circle

                        total_circle++;
                        c++; // counts the no of circlesso "c circle found at position
                       // System.out.println(c + "circle found in "+ i +"row" + "at position : " + starting_point + "  end:  " + end_point);

                        i = i + circle_dist; // distance between circles

                        //CandidateDetail stores the details of student info. part into the database	  
                        ...
                        if (((c % 15) > 0) && ((c % 15) <= 8)) // first 6 circles for rno
                        {
                           /*
                           obj.value = ""+count;
                           ...
                           */
                           ...
                           if (count == 10) {
                               ...
                               obj.field = "TID" + temp + " " + ((c % 15)-8);
                           } else {
                               obj.value = count;

                               obj.field = "TID" + count + " " + ((c % 15)-8);
                           }

                          /*  
                           if (count == 1) //first row
                            {
                               if ((c % 14) == 9) // circle at 7 position
                                {
                                   obj.field = "TestId";
                                   obj.value = "Arts";
                                  // i=i+200;//distance bw fac & sem
                                  // i= i+(ip.getWidth()*10)/100;
                               } else if ((c % 14) == 0) // circle at 8 position
                                {
                                   obj.field = "Semester";
                                   obj.value = "1";
                           ...
                           if (count == 2) // sec row
                            {
                               if ((c % 8) == 7) {
                                   obj.field = "faculty";
                                   obj.value = "commerce";
                                   i= i+(ip.getWidth()*10)/100;//distance bw fac & sem
          ...
          System.out.println("No of circles in a row: " + c);
       }

       System.out.println("total circles found : " + total_circle);

       for (CandidateDetail db : detail) {
          // System.out.println("field: " + db.field + "value:" + db.value);
          /*
           System.out.println("xend:" + db.xend);
           System.out.println("xstart:" + db.xstart);
           System.out.println("blockMidRatio:" + db.blockMidRatio);
    ...
    //--------------------------------------Answer Details-------------------------------------------------------//	 
    /**
     * This function process answer detail part of the base sheet
     * @param ip
     * @param infoLeft
     * @param infoRight
     * @param xrtAvg
...
System.out.println("x left avg  " + xleftAvg+ " x right Avg : "+ xrtAvg);
        //MidPoint contains the midpoint of each block 
        for (int j = 0; j < infoLeft.size(); j++) {
                ...
                if (ip.getPixelValue(i, y) == 0) {
                   // System.out.println("y :" + y + " y mid: " + ymidLeft.mp);
                    ...
                    if (checkPixelGroup(ip, i, y)) {
                        ...
                        posEndratio = ((end_point - xleftAvg) / (xrtAvg -
                            end_point));
                    	System.out.print("row :" + count+ " c : " + circles_in_row + " st : " + starting_point+ " END: " + end_point + "w: "+ (end_point-starting_point)+ "start ratio+ : " + posStartratio + "end ratio : " + posEndratio);
                        ...
                        //stores all the information in Ans_db object to insert into database 
            ...
            System.out.println("No. of circles in " +
            		count+" row : " + circles_in_row);
       }//end j loop
