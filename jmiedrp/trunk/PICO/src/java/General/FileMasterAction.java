 ...
 * @author kazim
 */
public class FileMasterAction extends DevelopmentSupport {

    private FileMaster fm = new FileMaster();
    private FileMaster fm_new = new FileMaster();
    ...
    public void setfm(FileMaster fm) {
        this.fm = fm;
    }

    public FileMaster getfm() {
        return this.fm;
    }

    public void setfm_new(FileMaster fm_new) {
        this.fm_new = fm_new;
    }

    public FileMaster getfm_new() {
        return this.fm_new;
    }
    ...
    public String SaveFileMaster() throws Exception {
        try {
            ...
            if(fm.getFileId() == null) {
                ...
                fm.setFileDispatchDate(d.convertStringToDate(fileDate));
                ...
                fm_new = fmDao.findFile(fm.getFileId());
                setDefaults(fm_new);
                
                setfileDate(d.convertDateToString(fm_new.getFileDispatchDate(), "dd-MM-yyyy"));

                message =   "File Created with Id :"  + fm_new.getDepartmentmaster().getDmShortName() + "/" +
                            fm_new.getFileNumber() + fm.getFileId() + "==" + fm_new.getFileId();
...
/*public String fetchFile() throws Exception {
    try {
        if(fm.getFileNumber().length() == 0)
            message = "Please enter File Number";
        else
            fmDao.findFile(defaultFileType)
        ...
        message = "Exception in -> FetchFileAxn" + e.getMessage() + " Reported Cause is: " + e.getCause();
        return ERROR;
    }
