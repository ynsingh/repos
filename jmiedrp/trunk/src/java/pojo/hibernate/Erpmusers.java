package pojo.hibernate;
// Generated May 29, 2011 11:33:37 PM by Hibernate Tools 3.2.1.GA


import java.util.Date;
import java.util.HashSet;
import java.util.Set;

/**
 * Erpmusers generated by hbm2java
 */
public class Erpmusers  implements java.io.Serializable {


     private Integer erpmuId;
     private String erpmuName;
     private String erpmuPassword;
     private Date erpmuDob;
     private String erpmuActive;
     private String erpmuVerifiedBy;
     private String erpmuSecretQuestion;
     private String erpmuSecretAnswer;
     private String erpmuFullName;
     private Set erpmIndentMastersForIndtUserId = new HashSet(0);
     private Set erpmIndentMastersForIndtForwardedToUserId = new HashSet(0);
     private Set erpmuserroles = new HashSet(0);
     private Set erpmPoMastersForPomUserId = new HashSet(0);
     private Set erpmPoMastersForPomApprovedById = new HashSet(0);

    public Erpmusers() {
    }

	
    public Erpmusers(String erpmuName, String erpmuPassword, Date erpmuDob, String erpmuActive, String erpmuFullName) {
        this.erpmuName = erpmuName;
        this.erpmuPassword = erpmuPassword;
        this.erpmuDob = erpmuDob;
        this.erpmuActive = erpmuActive;
        this.erpmuFullName = erpmuFullName;
    }
    public Erpmusers(String erpmuName, String erpmuPassword, Date erpmuDob, String erpmuActive, String erpmuVerifiedBy, String erpmuSecretQuestion, String erpmuSecretAnswer, String erpmuFullName, Set erpmIndentMastersForIndtUserId, Set erpmIndentMastersForIndtForwardedToUserId, Set erpmuserroles, Set erpmPoMastersForPomUserId, Set erpmPoMastersForPomApprovedById) {
       this.erpmuName = erpmuName;
       this.erpmuPassword = erpmuPassword;
       this.erpmuDob = erpmuDob;
       this.erpmuActive = erpmuActive;
       this.erpmuVerifiedBy = erpmuVerifiedBy;
       this.erpmuSecretQuestion = erpmuSecretQuestion;
       this.erpmuSecretAnswer = erpmuSecretAnswer;
       this.erpmuFullName = erpmuFullName;
       this.erpmIndentMastersForIndtUserId = erpmIndentMastersForIndtUserId;
       this.erpmIndentMastersForIndtForwardedToUserId = erpmIndentMastersForIndtForwardedToUserId;
       this.erpmuserroles = erpmuserroles;
       this.erpmPoMastersForPomUserId = erpmPoMastersForPomUserId;
       this.erpmPoMastersForPomApprovedById = erpmPoMastersForPomApprovedById;
    }
   
    public Integer getErpmuId() {
        return this.erpmuId;
    }
    
    public void setErpmuId(Integer erpmuId) {
        this.erpmuId = erpmuId;
    }
    public String getErpmuName() {
        return this.erpmuName;
    }
    
    public void setErpmuName(String erpmuName) {
        this.erpmuName = erpmuName;
    }
    public String getErpmuPassword() {
        return this.erpmuPassword;
    }
    
    public void setErpmuPassword(String erpmuPassword) {
        this.erpmuPassword = erpmuPassword;
    }
    public Date getErpmuDob() {
        return this.erpmuDob;
    }
    
    public void setErpmuDob(Date erpmuDob) {
        this.erpmuDob = erpmuDob;
    }
    public String getErpmuActive() {
        return this.erpmuActive;
    }
    
    public void setErpmuActive(String erpmuActive) {
        this.erpmuActive = erpmuActive;
    }
    public String getErpmuVerifiedBy() {
        return this.erpmuVerifiedBy;
    }
    
    public void setErpmuVerifiedBy(String erpmuVerifiedBy) {
        this.erpmuVerifiedBy = erpmuVerifiedBy;
    }
    public String getErpmuSecretQuestion() {
        return this.erpmuSecretQuestion;
    }
    
    public void setErpmuSecretQuestion(String erpmuSecretQuestion) {
        this.erpmuSecretQuestion = erpmuSecretQuestion;
    }
    public String getErpmuSecretAnswer() {
        return this.erpmuSecretAnswer;
    }
    
    public void setErpmuSecretAnswer(String erpmuSecretAnswer) {
        this.erpmuSecretAnswer = erpmuSecretAnswer;
    }
    public String getErpmuFullName() {
        return this.erpmuFullName;
    }
    
    public void setErpmuFullName(String erpmuFullName) {
        this.erpmuFullName = erpmuFullName;
    }
    public Set getErpmIndentMastersForIndtUserId() {
        return this.erpmIndentMastersForIndtUserId;
    }
    
    public void setErpmIndentMastersForIndtUserId(Set erpmIndentMastersForIndtUserId) {
        this.erpmIndentMastersForIndtUserId = erpmIndentMastersForIndtUserId;
    }
    public Set getErpmIndentMastersForIndtForwardedToUserId() {
        return this.erpmIndentMastersForIndtForwardedToUserId;
    }
    
    public void setErpmIndentMastersForIndtForwardedToUserId(Set erpmIndentMastersForIndtForwardedToUserId) {
        this.erpmIndentMastersForIndtForwardedToUserId = erpmIndentMastersForIndtForwardedToUserId;
    }
    public Set getErpmuserroles() {
        return this.erpmuserroles;
    }
    
    public void setErpmuserroles(Set erpmuserroles) {
        this.erpmuserroles = erpmuserroles;
    }
    public Set getErpmPoMastersForPomUserId() {
        return this.erpmPoMastersForPomUserId;
    }
    
    public void setErpmPoMastersForPomUserId(Set erpmPoMastersForPomUserId) {
        this.erpmPoMastersForPomUserId = erpmPoMastersForPomUserId;
    }
    public Set getErpmPoMastersForPomApprovedById() {
        return this.erpmPoMastersForPomApprovedById;
    }
    
    public void setErpmPoMastersForPomApprovedById(Set erpmPoMastersForPomApprovedById) {
        this.erpmPoMastersForPomApprovedById = erpmPoMastersForPomApprovedById;
    }




}


