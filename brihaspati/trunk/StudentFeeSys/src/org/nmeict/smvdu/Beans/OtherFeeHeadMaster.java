...
public class OtherFeeHeadMaster  implements java.io.Serializable {
     ...
     public OtherFeeHeadMaster(OrgProfile orgProfile, OrgDepartmentType orgDepartmentType, DegreeType degreeType,
             SemesterMaster semesterMaster, BranchMaster branchMaster, String feeHeadName, double feeHeadValue, 
             String programId, byte payableStatus, Set scholarshipTypes, Set studentMasters) {
	...
	public List<OtherFeeHeadMaster> getLoadAllFeeHead() {
		loadAllFeeHead = getiOrgOtherFeeHeadMasterService().loadOtherFeeHeadCode();
                //System.out.println("\n\n loadAllFeeHead=="+loadAllFeeHead.size());
	...
	public String saveFeeHead()
	{
                ...
                List<OtherFeeHeadMaster> otherFeeHeadMasterList = new ArrayList<OtherFeeHeadMaster>();
                otherFeeHeadMasterList = getiOrgOtherFeeHeadMasterService().loadOtherFeeHeadCode();
                boolean bl= false;
                    ...
                    if(fl.isStatus() == true){
                        ...
                        for(OtherFeeHeadMaster fhm : otherFeeHeadMasterList){
                            ...
                            if( fhm.getDepartmentCode().equals(this.getDepartmentCode())
                                ...
                                && fhm.getFeeHeadName().trim().equalsIgnoreCase(fl.getFeeHeadName().trim()) ){
                                                   
                                bl = true;
                                //System.out.println("222 fl.getFeeHeadName=="+fl.getFeeHeadName());
                                vc.add(fl.getFeeHeadName());
                                break;
                            }
                        }
                        ...
                        if(!bl){
                            try{
                                ...
                                if(fl.getFeeHeadValue() <= 0.0){
                                    msg = msg+"Please Enter '"+fl.getFeeHeadName()+"' Value Greater Than Zero.";
                                }
                                else{
                                                                 
                                    //System.out.println("3333");
                                    this.setFeeHeadName(fl.getFeeHeadName());
                                    this.setFeeHeadValue(fl.getFeeHeadValue());
                                    this.setFeeHeadId(fl.getFeeHeadId());
                ...
                try{
                    ...
                    if(active == vc.size()){
                        ...
                        message = new FacesMessage();
                        message.setSeverity(FacesMessage.SEVERITY_ERROR);
                        message.setSummary("All Given Fee Heads already exist.");
                        fc.addMessage("", message);
                        otherFeeHeadMasterList = null;
                        return null;  
                    }  
                    String feeHeads = "";
                    if( (vc.size() > 0) && (vc.size() < active) ){
                            
                        for(int i=0; i < vc.size(); i++){
                            feeHeads = feeHeads+","+ vc.get(i);
                        }
                        System.out.println("feeHeads=="+feeHeads);
                        message = new FacesMessage();
                        message.setSeverity(FacesMessage.SEVERITY_ERROR);
                        message.setSummary("From Given Fee Heads '"+feeHeads+"' Already exist. Rest Fee Heads Are Inserted Successfully.");
                        fc.addMessage("", message);
                        otherFeeHeadMasterList = null;
                        return null;     
                    }
	...
	public void update()
	{
            try{
                ...
                //System.out.println("Master Size=="+otherFeeHeadMasterList.size());
                int tempSize = otherFeeHeadMasterList.size();
                
                //System.out.println("otherFeeHead.size()="+otherFeeHead.size());
                int sizeLimit = otherFeeHead.size() - tempSize;
                
                if(sizeLimit > tempSize){
                    for(int i = 0; i < sizeLimit; i++){
                        
                        otherFeeHead.remove(tempSize);
                        //System.out.println("Size=="+otherFeeHead.size());
                    }
                     //System.out.println("iffffffffff size=="+otherFeeHead.size());
                }

                /*System.out.println("Size=="+otherFeeHeadMasterList.size());
                for(OtherFeeHeadMaster of: otherFeeHead){
                    System.out.println("\nfeeHeadValue ="+of.getFeeHeadValue());
                }*/
                getiOrgOtherFeeHeadMasterService().updateOtherFeeHeadCode(otherFeeHead);
                //getiOrgOtherFeeHeadMasterService().updateOtherFeeHeadCode(otherFeeHeadMasterList);
