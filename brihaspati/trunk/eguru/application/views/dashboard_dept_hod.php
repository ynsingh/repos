...
<html>
    <head>
        ...
        <script src="<?php echo base_url(); ?>/static/js/admin/dept_hod_approve.js"></script>
    </head>
    <body>
        <?php
        ...
        ?>
        <button data-toggle="modal" href="#edit" id="login" style="float:right;padding: 6px 12px" class="btn-large btn-success ">Edit Subjects &nbsp;<i class="icon icon-off icon-white icon-pencil"></i></button>
        <button data-toggle="modal" href="#add" id="login"style="float:right;padding: 6px 12px;position: relative;right:8px" class="btn-danger btn-large">Add Subject &nbsp;<i class="icon icon-off icon-white icon-plus"></i></button>
        <form  id="add_subject">
            <div id="add" class="modal hide fade in span6" style="display: none; ">
                ...
                <div class="modal-body span4">
                    <div class="control-group">                                                   
                        <div class="controls">
                            <input type="text" name="add_subject" class="required" placeholder="Subject Name" required/>
                        </div>
                    </div>                                                
                </div>
                ...
                <div class="modal-body span4">
                    <div class="control-group">                                                   
                        <div class="controls">
                            <?php
                            $count0 = 0;
                            foreach ($subjects->result_array() as $row0) {
                                $count0++;
                                ?>
                                <div>
                                    <input type="text" class="editting" name="add_subject<?= $count0 ?>" class="required" readonly value="<?= $row0['name'] ?>" required/>
        ...
        <div class="accordion" style="margin-top:65px">
            <?php $count = 1 ?>
            <?php foreach ($data_dash->result_array() as $row) {
                ?>
                <div class="accordion-group">
                    <div class="accordion-heading">
                        <a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion2" href="#collapse<?= $count ?>">
                            <h2><?php echo $row['first_name'] . " " . $row['last_name'] . "       Applied for:" . $row['name'];
                ?></h2>
                        </a>
                    </div>
...
<center>
<div class="" style=" background-color:antiquewhite;height: 40px;border-radius: 18px;margin-top:411px">
    <div class="span3"></div>
