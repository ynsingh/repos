...
function Logo()
	{
		...
		//$this->logo_path_url=base_url().'uploads/BGAS/logo';  
		/* Check access */
			...
			$idl='';
				...
				$ext = substr(strrchr($file_name, '.'), 1);
                        	...
                        	$config['file_name'] = $full_name ;
                                //$config['upload_path']=realpath(BASEPATH.'../uploads/BGAS/logo');
...
/*
				//start upload
					...
					$_FILES['userFile']['name'] = $_FILES['userfile']['name'];
                                        ...
                                        $config['upload_path'] = 'uploads/BGAS/logo/';
				...
				//	print_r($config);
						...
						$data = $this->upload->data();
                                               // print_r($data);
                                                //die;
                                                $this->messages->add('Error uploading  Institute logo', 'error');
                                               ...
                                               /* $picture = '';
                                                ...
                                                $ferror = $simsg.$ferror;
						$this->messages->add('logo not uploaded');
						redirect('setting/logo');

					}

*/
				//$this->upload_path='uploads/BGAS/logo/';
                		//$this->logo_path_url=base_url().'uploads/BGAS/logo';   
...
/*
						...
						//ploadData = $this->upload->data();
						//$data = array('upload_data' => $this->upload->data());
						//$uploadData = $this->upload->data();
                	   	 	       //icture = $uploadData['userfile'];
                                               //int_r($uploadData);
                                                //print_r($data);
                                                 //die;
						//$data = array('upload_data' => $this->upload->data());
						...
						else
						{
                					...
                					foreach ($error as $item => $value):
                      					$ferror = $item .":". $value;
                					endforeach;
                					$this->messages->add('error', $ferror);
							redirect('setting/logo');
           					 }		
