  ...
  //      $income = new Paymentreceipt();
	...
	if(!empty($result)){
		...
		foreach($result as $row){
			...
			echo "<td>";
			if($bal > 0){
				echo anchor('nodues/noduesnote/'.$row['secunit'].'/'.$bal, "Issue No Dues Notes") ;
				//				echo "Issue No Dues Notes";
			}
			else{
				echo anchor('nodues/noduescert'.$row['secunit'].'/'.$bal, "Issue No Dues Certificate") ;
				//echo "Issue No Dues Certificate";
			}
			echo "</td>";
			echo "</tr>";
			$i++;
