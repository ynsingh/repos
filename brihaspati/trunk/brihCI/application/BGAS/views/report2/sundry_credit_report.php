...
<?php
	echo form_open('report2/sundry_credit_report/'. $party_id);
    	echo "<p>";
	echo form_label('Party Name', 'party_id');
 	...
 	echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
   	echo form_submit('submit', 'Show');
        $help_uri = site_url()."/help/helpdoc#SundryCreditorsReport";
	...
	$page_count = $this->security->xss_clean($page_count);
    	if ( ! $page_count)
      		$page_count = "0";
 	$config['base_url'] = site_url('report2/sundry_credit_report/'.$party_id);
	...
	{
		$this->db->distinct();
              	$this->db->select('secunitid')->from('entry_items')->like('ledger_code','10040106','after');
               	$sec_count = $this->db->get();
       		$config['total_rows'] = (int)$sec_count->num_rows;
	}
	else
       		$config['total_rows'] = (int)$this->db->from('entry_items')->where('secunitid',$party_id)->like('ledger_code','10040106','after')->count_all_results();
	...
	else
	{
		if($party_id == 0)
		{
			$this->db->select('entry_id')->from('entry_items')->like('ledger_code','10040106','after');
			...
			if($cred_rep->num_rows == 0)
			{
				...
				if( $cred_rep1->num_rows() < 1 )
					...
					echo "<table width=\"100%\" border=\"0\">";
                			...
                			echo "<tr style = \"text-align:center;\">";
                			echo "<td style=\"padding: 15px 15px 15px 15px;background-color:#87CEEB;\">";
                			echo "<h3>There is no Sundry credit Report for any party</h3>";
                			echo "</td>";
                			echo "</tr>";
			...
			{
				$end_uri = site_url()."/report2/sundry_credit_report/";
				$this->db->distinct();
				$this->db->select('secunitid')->from('entry_items')->like('ledger_code','10040106','after');
				...
				foreach($cred_rep_sec->result() as $row2)
					...
					$total_cr = $this->Secunit_model->get_sundry_seccr_secdr_total($row2->secunitid,"C","10040106");
					$total_dr = $this->Secunit_model->get_sundry_seccr_secdr_total($row2->secunitid,"D","10040106");
			...
			if($cred_rep->num_rows == 0)
			{
				...
				if( $cred_rep1->num_rows() < 1 )
					...
					echo "<table width=\"100%\" border=\"0\">";
                			...
                			echo "<tr style = \"text-align:center;\">";
                			echo "<td style=\"padding: 15px 15px 15px 15px;background-color:#87CEEB;\">";
                			echo "<h3>There is no Sundry credit Report for the party " . $p_nme . "</h3>";
                			echo "</td>";
                			echo "</tr>";
