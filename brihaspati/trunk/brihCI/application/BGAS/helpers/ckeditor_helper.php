...
function cke_initialize($data = array()) {
	...
	if(!defined('CI_CKEDITOR_HELPER_LOADED')) {
		...
		$return =  '<script type="text/javascript" src="'.asset_url().'assets/bgas/' . $data['path'] . '/ckeditor.js"></script>';
		$return .=	"<script type=\"text/javascript\">CKEDITOR_BASEPATH = '" . asset_url() . 'assets/bgas/'.$data['path'] . "/';</script>";
		//$return =  '<script type="text/javascript" src="'.asset_url(). 'assets/bgas/js/ckeditor'. $data['path'] . '/ckeditor.js"></script>';
            
		//$return .=	"<script type=\"text/javascript\">CKEDITOR_BASEPATH = '" . asset_url() . 'assets/bgas/js/ckeditor'. $data['path'] . "/';</script>";
	} 
	    ...
	    foreach($data['styles'] as $k=>$v) {
	    	...
	    	if(isset($v['styles'])) {
	    		foreach($v['styles'] as $k2=>$v2) {
	    			...
	    			$return .= "'" . $k2 . "' : '" . $v2 . "'";
	    			$segment = array_keys($data['styles']);
   				 if($k !== end($segment)) {
			           $return .= ",";
    	}	
