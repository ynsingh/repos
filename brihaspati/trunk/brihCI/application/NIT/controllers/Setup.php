...
//	$data['prgres'] = $getres;
    ...
    public function editprogram($prgid){
        ...
        /*$data['prgcampus'] = array(
            ...
            'id' => 'prgcampus',
            'size' => '40',
            'value' => $this->common_model->get_listspfic1('org_profile','org_name','org_id',$program_data->prg_scid)->org_name,
            'readonly'=>'true',
            );*/
...
//*************************Start Department**************************************//
 	public function dept(){
		...
		$is_exist = $this->common_model->isduplicatemore('Department',$datawh);
		if(!$is_exist){
                 ...
                 //	if (($_POST['orgprofile'] != '') || ($_POST['studycenter'] != '')){  
                   	 ...
                   	 }//if close post orgprofile and studycenter
		//}//duplicate if close	
		else{
			$this->logger->write_logmessage("insert","Trying to add Department  ", "Department is not added because combination is already exist ");
                        $this->logger->write_dblogmessage("insert","Trying to add fees with head", "Department is not added because combination is already exist ");
                        $this->session->set_flashdata('err_message','Department'.$_POST['dept_name'].'is already exsist');
                        redirect('setup/dispdepartment');	
		}
		return;
                }//validation if close
...
*/
	...
	$prgid = $this->common_model->get_listspfic1('program','prg_id','prg_name',$prgname)->prg_id;	
	$insertdata= array(
...
/**This function is used for update record of  enterance exam fees 
   ...
   public function set_datesview() {  
	...
	public function set_datesadd(){
		...
		//$data['campuslist'] = $this->common_model->get_list('org_profile');  
		$data['currentacadyear'] = $this->datemodel->getcurrentAcadYear();
	...
	$updateset_date = array(
	  ...
	  $logmessage = "";
            if($data['acady'] != $_POST['editacad_year'])
                    $logmessage = "Academic Year ".' ' .$data['acady']. " changed  " .$_POST['editacad_year'];
            if($data['sem'] != $_POST['editsemester'])
                     $logmessage = "Edit Semester " .' '.$data['sem']. " changed  " .$_POST['editsemester'];
            //if($data['campuscode'] != $_POST['editcampus_code'])
              //  $logmessage = "Edit Campus Code".' ' .$data['campuscode']. " changed  " .$_POST['editcampus_code'];
            if($data['session_sdate'] != $_POST['editsession_sdate'])
                $logmessage = "Edit Start Session Date" .' '.$data['session_sdate']. " changed  " .$_POST['editsession_sdate'];
            if($data['session_edate'] != $_POST['editsession_edate'])
                $logmessage = "Edit End Session Date" .' '.$data['session_edate']. " changed  " .$_POST['editsession_edate'];
            if($data['exam_sdate'] != $_POST['editexam_startdate'])
                $logmessage = "Edit Start Exam Date" .' '.$data['exam_sdate']. " changed  " .$_POST['editexam_startdate'];
            if($data['exam_edate'] != $_POST['editexam_enddate'])
                $logmessage = "Edit End Exam Date" .' '.$data['exam_edate']. " changed  " .$_POST['editexam_enddate'];
            if($data['marks_sdate'] != $_POST['editform_sdate'])
                $logmessage = "Edit Start Marks Submission Date" .' '.$data['marks_sdate']. " changed  " .$_POST['editform_sdate'];
            if($data['marks_edate'] != $_POST['editform_edate'])
                $logmessage = "Edit End Marks Submission Date" .' '.$data['marks_edate']. " changed  " .$_POST['editform_edate'];
