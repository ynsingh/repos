   ...
   <table width="100%;">
	...
	<?php //$count = 0;
	 ...
	 if( count($this->result) ) {
		 ...
		 {
			/*if($scid !=$row->pstp_scid){
			?>
				<tr>
					<td colspan=10 style="font-size:18px;text-align:center;">
					<b>University Name :</b> 
                        		<?php
                          		echo $this->commodel->get_listspfic1('org_profile','org_name','org_id',$row->pstp_scid)->org_name;
                        		?></td>
				</tr>
                     	<?php $scid =$row->pstp_scid;
			
			}*/
			$deptid = $this->commodel->get_listspfic1('user_role_type', 'deptid', 'userid', $row->pstp_teachid)->deptid;
			if($departid !=$deptid){
			?>
				<tr>
