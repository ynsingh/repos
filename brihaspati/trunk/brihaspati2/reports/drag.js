...
var multiSlotsInOrder;
function save(oForm) {
	try{
		multiSlotsInOrder = true;
		oForm.elements['eventSlotMap'].value = getEventAndSlot();
		if(multiSlotsInOrder){
			//alert(oForm.elements['eventSlotMap'].value);
			//throw "Erro";
			oForm.submit();
			return false;
		}
			...
			oForm.action = '';
			alert("There is an error in arranging one or more multi-slot events. Please make sure they are in order. All changes will be reset now.");
		}
	} catch(err) {
		alert(err);
	}
}
function getEventAndSlot(){
	var eventSlotMap = '';
	var divArray = document.getElementsByTagName("td");
	var multiSlot = new Array();
	var xx = 0;
	for (var i = 0; i<divArray.length; i++){
		var id = divArray[i].id;
		if (id!=null && id.length>9){
			var subid = id.substring(0, 10);
			if(subid == "tableSlot="){
				var innerDiv = divArray[i].getElementsByTagName('div');
				if (innerDiv!=null && innerDiv.length==1) {
					var innerDivId = innerDiv[0].id;
					if(innerDivId!=null && innerDivId.length>8){
						var subInnerid = innerDivId.substring(0, 8);
						if (subInnerid == "eventId=") {
							var index = innerDivId.indexOf('&');
							var typeE = innerDivId.substring(index+1, index+6);
							if(typeE == "type=") {
								var len = innerDivId.substring(index+6);
								var tableSlot = id.substring(10);
								var eventId = innerDivId.substring(8, index);
								eventSlotMap += eventId;
								eventSlotMap += '::';
								eventSlotMap += tableSlot;
								eventSlotMap += '::::';
			/////////////////////////////////Just to Reset Tab///////////////////////////////////
			if(len==3 || len==2){
				if(!multiSlot.exists(eventId)){
					for(var k = 0; k<len; k++) {
						try{
							var nextSlot = tableSlot++;
							var nextInnerDivId = document.getElementById(subid+nextSlot).getElementsByTagName('div')[0].id;
							if(nextInnerDivId!=innerDivId){
								multiSlotsInOrder = false;
							}
						} catch(err) {
							multiSlotsInOrder = false;
						}
					}
					multiSlot[multiSlot.length] = eventId
				}
			}
			///////////////////////////////////////////////////////////////////////////////////
							}
