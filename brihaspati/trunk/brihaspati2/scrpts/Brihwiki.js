...
BrihWiki.Editor = function() {
	...
	return {
		initButton: function(buttonId, tagOpen, tagClose, sampleText) {
			...
			document.getElementById(buttonId).onclick = insertTags.bind(BrihWiki.Editor, tagOpen, tagClose, sampleText);
		}
	};
}();
BrihWiki.UI = function() {
	function toggleToc(toggleLink, toggleTarget, hideLabel, showLabel) {
		if (toggleLink.innerHTML == hideLabel) {
			...
			BrihWiki.UI.addClass(toggleTarget, "hidden");
		} else {
			toggleLink.innerHTML = hideLabel;
			BrihWiki.UI.removeClass(toggleTarget, "hidden");
		}
		return false;
	}
	return {
		...
		addClass: function(element, cssClassName) {
			if (!BrihWiki.UI.hasClass(element, cssClassName)) {
		...
		toggleClass: function(element, cssClassName) {
			if (BrihWiki.UI.hasClass(element, cssClassName)) {
				BrihWiki.UI.removeClass(element, cssClassName);
			} else {
				BrihWiki.UI.addClass(element, cssClassName);
			}
		},
		...
		initializeToggle: function(containerElement, toggleElement, expandedClass) {
			// close by default
			BrihWiki.UI.removeClass(containerElement, expandedClass);
			toggleElement.onclick = BrihWiki.UI.toggleClass.bind(BrihWiki.UI, containerElement, expandedClass);
		},
...
BrihWiki.Admin = function() {
	...
	function toggleDisableOnChange(selectElement, enableValue, idElements, containerElement, expandedClass) {
		...
		if (disabled) {
			...
			BrihWiki.UI.addClass(containerElement, expandedClass);
		}
	}
	...
	return {
		initializeVirtualWikiCheckboxes: function() {
			for (var i = 0; i < virtualWikiCheckboxArray.length; i++) {
				...
				checkbox.onclick = toggleInputState.bind(BrihWiki.Admin, checkbox, input);
...
window.onload = function() {
	BrihWiki.Tabs.initializeTabs();
	BrihWiki.Admin.initializeVirtualWikiCheckboxes();
}
