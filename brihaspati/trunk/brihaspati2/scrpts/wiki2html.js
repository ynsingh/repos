(function () {
	...
	// utility function to check whether it's worth running through the wiki to html
	function iswiki(s) {
    		if (extendString) {
        		s = this;
    		}
	...
	function wiki2html(s) {
		...
		.replace(/'''(.*?)'''/g, function (m, l) { // bold
        	...
        	.replace(/\[\[(.*?)\]\]/g, function (m, l) { // internal link or image
				...
				var urUrl = "/brihaspati/servlet/brihaspati/template/call%2CWiki%2CWiki_Home.vm";
				//urUrl=urUrl.replace("Wiki_Home.vm","Editwiki.vm");
				var url=urUrl+"/filename/"+link;
                		return '<a href="'+url + '">' + (p.length ? p.join('|') : link) + '</a>';
	            	}
        	})
	    );
	}
