...
public class PresentationAudioReceive implements ReceiveStreamListener, SessionListener {
    	...
    	private static PresentationAudioReceive hav=null;
    	private static DataSource ds=null;
    	private int port=RuntimeDataObject.getController().getPresentationAudioPort();//(ClientObject.getController().getAVPort())+4;
	...
	/**Initialise the RTPSession for the Receving of unicast audio from the same or the remote machine */
		...
		if (!dataReceived) {
	    		System.out.println("No handraise RTP data was received.");
            		JOptionPane.showMessageDialog(null,Language.getController().getLangValue("PresentationAudioReceive.MessageDialog1"));
        	...
        	JOptionPane.showMessageDialog(null,Language.getController().getLangValue("PresentationAudioReceive.MessageDialog2"));
        	return true;
    	}
