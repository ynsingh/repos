...
/**
 * @author <a href="mailto:ashish.knp@gmail.com">Ashish Yadav </a>Created on 10Sep2012 
 */
...
public class ProxyAuthenticator implements ActionListener {
	...
	public void createGUI(){
		frame=new JFrame();
                frame.setTitle("Proxy Authentication Required");
                main_panel=new JPanel();
                main_panel.setLayout(new BorderLayout());

                north_panel=new JPanel();
                textLabel=new JLabel("The proxy"+RuntimeObject.getController().getProxyHost()+":"+RuntimeObject.getController().getProxyPort()+"is requesting a username and password");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getActionCommand().equals("OK_Bttn")){
			if(!((usr_text.getText()).equals("")) && (!(usr_pass.getText()).equals(""))){
                        	RuntimeObject.getController().setProxyUser(usr_text.getText());
                              	RuntimeObject.getController().setProxyPass(usr_pass.getText());
                             	frame.dispose();
                      	}else{
