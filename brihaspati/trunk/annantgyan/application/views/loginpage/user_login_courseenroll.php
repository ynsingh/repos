...
<form action="<?php echo site_url('login/user_login_courseenroll');?>" method="POST" >
	<div class="col-md-7">
	<?php
				if(!empty($course_data)){
					$i=0;
					foreach ($course_data as $row) {
						$crsreg[$i] = $row->uct_courseid;
						$i++;
					}
				}	
				$flag=true;
?>
		<label style="font-size: 20px;color: #79522f;">Select Course :</label>
...
<?php
				if(!empty($coursedate)){
					foreach($coursedate as $coudata){
						$crsannid = $coudata->crsann_crsid;
						if(!(in_array($crsannid, $crsreg))){
							 $cname = $this->commodel->get_listspfic1('courses','cou_name','cou_id',$crsannid)->cou_name;
...
<?php
				if($flag){
					$this->session->set_flashdata('error'.'No new course Open for Registration');
