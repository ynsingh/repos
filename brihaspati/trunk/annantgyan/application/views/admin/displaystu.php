...
-->
		<form name="frm" id="frmid" action="<?php echo site_url('admin/displaystu')?>" method="POST" enctype="multipart/form-data">
		...
		<table class="table table-bordered">
			<thead style="font-size: 18px;">
				<tr  class="info">
					<th>Sr. No.</th><th>Test Name</th><th>Attempted/Not Attempted </th><th>Marks Obtained</th>
				</tr>
			</thead>
			<tbody>
				<?php 
						...
						if(!empty($studata)){
							foreach($studata as $row){	
								$tname = $this->commodel->get_listspfic1('test','testname','testid',$row->st_testid)->testname;
								$tcode = $this->commodel->get_listspfic1('test','testcode','testid',$row->st_testid)->testcode;
						?>
				<tr>
					
								<td><?php echo $i++;?></td>
								<td><?php echo $tname ." ( ". $tcode ." )";?></td>
								//check entry exist in student test
								<td><?php echo "Attempted";?></td>
								<td><?php echo $row->st_marks;?></td>
								//if not
								<td><?php 
								$totm = $totm + $row->st_marks;
