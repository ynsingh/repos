...
public class  BufferMgt {
	...
	private java.util.LinkedList<byte[]> audio_data=new java.util.LinkedList<byte[]>();
	...
	* is now shifted to the end of last packet location in buffer.
 	*/
		try {
                       	Vector pointer=pointerStore_userRemove.getAllPointer();
	...
	/**
	 * This method is used to get data from buffer and increase pointer in the pointer buffer for this userid.
	 */  
		...
		try {	
			...
			while( cur_position_pointer < size) {
				...
				String get_userid=(buffer.getSourceUser_id(cur_position_pointer)).toString();
					...
					str=buffer.getDataObject(cur_position_pointer);
					if(str != null) 
	...
	/**
 	 *This method is used to get Audio_data from buffer and increase pointer in the pointer buffer for this userid.
	 */
		...
		try {
			...
			int cur_position_pointer = pointerStore_userRemove.getPointerBy_UserId(user_id);
                        ...
                        while( cur_position_pointer < size) {
                                String get_userid=(buffer.getSourceUser_id(cur_position_pointer)).toString();
                                if(!(get_userid.equals(user_id))) {
                                        byte[] str=buffer.getDataObject(cur_position_pointer);
                                        if(str != null){
						audio_data.addLast(str);
                                                pointerStore_userRemove.setPointer(user_id,++cur_position_pointer);
					}
                                } else
                                        pointerStore_userRemove.setPointer(user_id,++cur_position_pointer);
                                if(audio_data.size()>50)
                                        break;
                        }
                        removeBufferAndSetPointer();
			int currentOffset = 0;
         ...
         /**
          * Create putByte method to insert the packets in appropriate queue after matching packet type and queue type.                
          */                  

	 public synchronized void putByte(byte[] data,String user_id){
		try {
                        buffer.putDataObjectAndUserId(user_id, data);
		}catch(Exception e){ System.out.println("Exception in putByte method in BufferMgt class "+e.getMessage()); }
	}
	
	/**
 	 * Create Audio_byte method to insert the packets in appropriate queue after matching audio_packet type and queue type.
	 */
