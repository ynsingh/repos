...
* @see template Launchcontentnew.vm
* @author <a href="mailto:palseema30@gmail.com">Manorama pal</a>
* @author <a href="mailto:kishore.shukla@gmail.com">Kishore kumar shukla</a>
* @Created Date: 18-01-2012

*/
//SCO NAVIGATION FUNCTION
function goToSco(i) {
        frame_navigation.goToSco(i);
       // alert("frame_navigation.goToSco(i)");
}
...
function URLLoadTextDocs(entryNo)
{
    var str1 = theMenu.entry[entryNo].url.split('/');
    var item = (str1.length>1) ? str1[str1.length-1] : theMenu.entry[entryNo].url;
    parent.frame_content.location.href = item;
    parent.frame_content.location.reload();
}

function toggleChild(entryNo)
{
    for (level=theMenu.entry[entryNo].parent; level>-1; )
    {
        if ( theMenu.entry[level].isopen == false )
            eval(theMenu.toggle(level));
        level = theMenu.entry[level].parent;
    }
    highlightFocus(entryNo);
}
function highlightFocus(entryNo)
{
    var x = self.menu.document.links.length;
    var str1 = theMenu.entry[entryNo].url.split('/');
    var item = (str1.length>1) ? str1[str1.length-1] : theMenu.entry[entryNo].url;
    for ( xloc = 0; xloc<x; xloc++ )
    {
        if ( str1[str1.length-1]=='#')
            continue;
        var str = self.menu.document.links[xloc].href.split('/');
        if ( str[str.length-1]==item)
        {
            self.menu.document.links[xloc].focus();
            return ;
        }
    }
}
...
{
    var entryNo = (theMenu.selectedEntry==-1) ? (type=='next')?-1:0 : theMenu.entry[theMenu.selectedEntry].id;
    if ( (entryNo==theMenu.count && type=='next') || (entryNo==0 && type=='prev') )
        return ;
    eval(theMenu.itemClicked((type=='next')?++entryNo:--entryNo));
    //URLLoadTextDocs(entryNo);
    toggleChild(entryNo);
}
function highlightFirstItem() {
        var entryNo = 0;
        eval(theMenu.itemClicked(entryNo));
        toggleChild(entryNo);
}
...
function MIGetDocIconName() {
        ...
        ////////////////////////add this to open the folder icon
        n += ((this.isopen) && (this.FirstChild != -1)) ? 'O' : '';
        //////////////////////
...
function Menu () {
        ...
        this.contentFrame = 'frame_content';
        this.defaultTarget = 'frame_content';
        this.tipText = 'none';
...
function initOutlineIcons(imgStore) {
	var ip = '/brihaspati2/images/menu/';
	//alert("ip========="+ip);
        ...
        imgStore.add('FolderO', ip + 'folderopen.gif', 18, 16);
        ////////////////////////
        var di = '/brihaspati2/images/menu/win/';
	//alert("di========"+di);
        if ((theBrowser.code == 'NS') || (theBrowser.code == 'MSIE')) {
                //di += theBrowser.code.toLowerCase(); // + '_doc';
                imgStore.add('Document', di + 'document.gif', 18, 16);
                imgStore.add('DocumentMouseOver', di + 'document_mo1.gif', 18, 16);
                imgStore.add('FolderMouseOver', di + 'folderclosed3.gif', 18, 16);
                imgStore.add('FolderOMouseOver', di + 'folderopen3.gif', 18, 16);
                imgStore.add('DocumentSelected', di + 'document3.gif', 18, 16);
                imgStore.add('FolderSelected', di + 'folderclosed2.gif', 18, 16);
                imgStore.add('FolderOSelected', di + 'folderopen2.gif', 18, 16);
                imgStore.add('ref01', di + 'ref01.gif', 18, 16);
        } else {
                imgStore.add('Document', di + 'document.gif', 18, 16);
        }
}
//This is where items are putted
function initialise(API) {
        ...
        theMenu.contentFrame = "frame_content";
        theMenu.defaultTarget = "frame_content";
        ...
        //TREE MENU HERE !!
        var menutype = "Folder";
        //level1ID = theMenu.addMenu(-1, menutype, title, "", "");
        //theMenu.entry[level1ID].isopen = true;
        var total = API.getTotalSco();
	//alert("total===="+total);
for ( i = 0; i < total; i++ ) {
                var s = i + 1;
                var sectitle = API.getTitle(i);
                var level = API.getScoLevel(i);
                var nextLevel = level;
                if ( i < total - 1 ) {
                        nextLevel = API.getScoLevel(i+1);
                }
                if ( nextLevel > level )
                        menutype = "Folder";
                else
                ...
                if ( i == 0 ) {
                        level1ID = theMenu.addMenu(-1, menutype, sectitle, "javascript:parent.goToSco(" + i + ")", "");
                        theMenu.entry[level1ID].isopen = true;
                } else {
                        if ( level == 0 )
                                level1ID = theMenu.addMenu(level1ID, menutype, sectitle, "javascript:parent.goToSco(" + i + ")", "");
                        else if ( level == 1 )
                                level2ID = theMenu.addChild(level1ID, menutype, sectitle, "javascript:parent.goToSco(" + i + ")", "");
                        else if ( level == 2 )
                                level3ID = theMenu.addChild(level2ID, menutype, sectitle, "javascript:parent.goToSco(" + i + ")", "");
                        else if ( level == 3 )
                                level4ID = theMenu.addChild(level3ID, menutype, sectitle, "javascript:parent.goToSco(" + i + ")", "");
