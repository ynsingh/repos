...
<table  border="0"  style="position:absolute; left: 0%; top: 10%; font-size: 12px;" dir="<%=rtl %>">
<div>
<%
if(tcount==0)
{
...
<ui:dataGrid items="${opacList}" var="doc" name="datagrid1" cellPadding="0" cellSpacing="0" styleClass="datagrid" >
  ...
  <columns>
    ...
    <column width="100">
      ...
      <item   value="${view}"  hAlign="left" hyperLink="${path}/cataloguing/showMarcDetails1.do?id=${doc.id.biblioId}"
	      styleClass="item"/>
    </column>
      <column width="100">
      <header value="${action}" hAlign="left" styleClass="admingridheader"/>
      <item   value="Export data"  hAlign="left" hyperLink="${path}/exportMarc.do?id=${doc.id.biblioId}"
	      styleClass="item"/>
    </column>
  </columns>
