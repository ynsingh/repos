...
int pagesize=100;
    ...
    int tcount =0;
     ...
     opacList = new ArrayList ();
        ...
        result = (ArrayList)session.getAttribute("AccList");
        from=i;
        int j=0;
        while(j<result.size())
        {
            DocumentDetails obj=(DocumentDetails)result.get(j);
            OpacDoc obj1=new OpacDoc();
            obj1.setBiblioid(obj.getBiblioId());
            obj1.setTitle(obj.getTitle());
            obj1.setCallno(obj.getCallNo());
            obj1.setRowno(i+1);
            obj1.setLibrary_id(obj.getId().getLibraryId().toUpperCase());
            obj1.setAccessionno(obj.getAccessionNo());
...
String msg=(String)request.getAttribute("msg");
  ...
  <script type="text/javascript" language="javascript">
     <% if (msg!=null){%>
                    alert("<%=msg%>");
                    top.window.location.href="<%=request.getContextPath()%>/cataloguing/acc_search.jsp";
                    <%}%>
                         <% if (msg1!=null){%>
                    alert("<%=msg1%>");
                    top.window.location.href="<%=request.getContextPath()%>/cataloguing/acc_search.jsp";
...
<column width="20%">
 ...
 <column width="20%">
      ...
      <item   value="Edit"  hAlign="left" hyperLink="${path}/cataloguing/updateacc.do?id=${doc.accessionno}&amp;ac=up"
	      styleClass="item"/>
    </column>
      
<column width="15%">
      <header value="${action}" hAlign="left" styleClass="admingridheader"/>
      <item   value="Delete"  hAlign="left" hyperLink="${path}/cataloguing/updateacc.do?id=${doc.accessionno}&amp;ac=de"
	      styleClass="item"/>
    </column>
