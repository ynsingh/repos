<%--
    Document   : delete_voter
    Created on : 21 Dec, 2013, 11:32:41 AM
    Author     : Farooq
--%>
 ...
 //  toIndex = tcount;
  ...
  // if(opacList!=null)request.setAttribute ("opacList", opacList.subList(fromIndex, toIndex));
   ...
   status = "&status="+status;
    pageContext.setAttribute("status",status);
    String bt="&bt="+"dt";
     pageContext.setAttribute("bt",bt);
     pageContext.setAttribute("tCount", tcount);
...
<ui:dataGrid items="${requestList}"  var="doc" name="datagrid1" cellPadding="0" cellSpacing="0" styleClass="datagrid">
  ...
  <columns>
    ...
    <column width="10%">
      ...
      <item   value="${doc.enrollment}" hyperLink="${path}/newregistration2.do?id=${doc.enrollment}${status}${bt}"  hAlign="left"    styleClass="item"/>
    </column>

    <column width="10%">
      <header value="${Voter_Name}" hAlign="left" styleClass="header"/>
      <item   value="${doc.voter_name}" hAlign="left" hyperLink="${path}/newregistration2.do?id=${doc.enrollment}${status}${bt}"  styleClass="item"/>
    </column>
    <column width="10%">
      <header value="Email" hAlign="left" styleClass="header"/>
      <item   value="${doc.email_id}" hAlign="left" hyperLink="${path}/newregistration2.do?id=${doc.enrollment}${status}${bt}"  styleClass="item"/>
    </column>

    <column width="10%">
      <header value="${Course}" hAlign="left" styleClass="header"/>
      <item   value="${doc.course}" hyperLink="${path}/newregistration2.do?id=${doc.enrollment}${status}${bt}"  hAlign="left" styleClass="item"/>
    </column>


       <column width="10%">
      <header value="${Year}" hAlign="left" styleClass="header"/>
      <item   value="${doc.year}" hyperLink="${path}/newregistration2.do?id=${doc.enrollment}${status}${bt}"  hAlign="left" styleClass="item"/>
    </column>


      <column width="10%">
      <header value="${Status}" hAlign="left" styleClass="header"/>
      <item   value="${doc.status}" hyperLink="${path}/newregistration2.do?id=${doc.enrollment}${status}${bt}"  hAlign="left" styleClass="item"/>
    </column>


      <column width="10%">
      <header value="Action" hAlign="left" styleClass="header"/>
      <item   value="Delete" hyperLink="${path}/newregistration2.do?id=${doc.enrollment}${status}${bt}"  hAlign="left" styleClass="item"/>
    </column>
