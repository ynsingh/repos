...
<%
    String msg2 =(String)session.getAttribute("msg1");
     ...
     <script type="text/javascript" language="javascript">
         alert("<%=msg2%>");
         top.location.href="<%=request.getContextPath()%>/election_manager/search_voter.jsp?status=B";
 ...
 Ob=(List<ElectionDetails>)session.getAttribute("resultset");
...
//requestList = (Login)rs.get(0);
 ...
 /*  while (it.hasNext()) {
      ...
      Ob = (List<ElectionDetails>)rs.get(tcount);
...
<ui:dataGrid items="${requestList}"  var="doc" name="datagrid1" cellPadding="0" cellSpacing="0" styleClass="datagrid">
  ...
  <columns>
    ...
    <column width="10%">
      ...
      <item   value="${doc.voterRegistration.id.enrollment}" hyperLink="${path}/newregistration2.do?id=${doc.voterRegistration.id.enrollment}${status}"  hAlign="left"    styleClass="item"/>
    </column>

      <column width="10%">
      <header value="Election Name" hAlign="left" styleClass="header"/>
      <item   value="${doc.election.electionName}" hyperLink="${path}/newregistration2.do?id=${doc.voterRegistration.id.enrollment}${status}"  hAlign="left"    styleClass="item"/>
    </column>

    <column width="10%">
      <header value="${Voter_Name}" hAlign="left" styleClass="header"/>
      <item   value="${doc.voterRegistration.voterName}" hAlign="left" hyperLink="${path}/newregistration2.do?id=${doc.voterRegistration.id.enrollment}${status}"  styleClass="item"/>
    </column>

      <column width="10%">
      <header value="Email" hAlign="left" styleClass="header"/>
      <item   value="${doc.voterRegistration.email}" hAlign="left" hyperLink="${path}/newregistration2.do?id=${doc.voterRegistration.id.enrollment}${status}"  styleClass="item"/>
    </column>

    <column width="10%">
      <header value="${Course}" hAlign="left" styleClass="header"/>
      <item   value="${doc.voterRegistration.course}" hyperLink="${path}/newregistration2.do?id=${doc.voterRegistration.id.enrollment}${status}"  hAlign="left" styleClass="item"/>
    </column>


       <column width="10%">
      <header value="${Year}" hAlign="left" styleClass="header"/>
      <item   value="${doc.voterRegistration.year}" hyperLink="${path}/newregistration2.do?id=${doc.voterRegistration.id.enrollment}${status}"  hAlign="left" styleClass="item"/>
    </column>


      <column width="10%">
      <header value="${Status}" hAlign="left" styleClass="header"/>
      <item   value="${doc.voterRegistration.status}" hyperLink="${path}/newregistration2.do?id=${doc.voterRegistration.id.enrollment}${status}"  hAlign="left" styleClass="item"/>
    </column>

       <column width="10%">
      <header value="Action" hAlign="left" styleClass="header"/>
      <item   value="Unblock" hyperLink="${path}/unblockvoter.do?id=${doc.voterRegistration.id.enrollment}&amp;eid=${doc.election.id.electionId}"  hAlign="left" styleClass="item"/>
    </column>

      <column width="10%">
      <header value="Action" hAlign="left" styleClass="header"/>
      <item   value="Send Mail" hyperLink="${path}/election_manager/VoterMailBody.jsp?id=${doc.voterRegistration.id.enrollment}"  hAlign="left" styleClass="item"/>
    </column>
...
<tr><td>
 ...
 <input type="button" onclick="send()" value="next" class="datagrid"/>
  <input type="button" onclick="sendprevious()" value="previous" class="datagrid"/>
   ...
   <input type="text" class="datagrid" size="50" readonly value="Total Number of Voters Per Page: <%=Ob.size()%>" /></td></tr>
