/*
 TO PRINT REPORT OF CURRENT NOMINATION LIST OF CANIDATE WHOSE REQUEST IS NOT ACCECCPTED
 */
 ...
 * @author Edrp-04
 */
public class PrintNomination extends org.apache.struts.action.Action {
  ...
  private static Logger log4j=LoggerUtils.getLogger();
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
           ...
           try{
            ...
            JasperCompileManager.compileReportToFile(path+"NominationReport.jrxml");
            String institute_id=(String)session.getAttribute("institute_id");
            List  list=dao.NominationReport(institute_id);
            JRBeanCollectionDataSource data=new  JRBeanCollectionDataSource(list);
            ServletOutputStream ouputStream = response.getOutputStream();
            response.setContentType("application/pdf");
            HashMap hash= new HashMap();
            JasperFillManager.fillReportToFile(path+"NominationReport.jasper",hash,data);
            File file= new File(path+"NominationReport.jrprint");
            JasperPrint print =(JasperPrint)JRLoader.loadObject(file);
            JRPdfExporter pdf=new JRPdfExporter();
            pdf.setParameter(JRExporterParameter.JASPER_PRINT, print);
            pdf.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, path+"NominationReport.pdf");
            pdf.exportReport();
